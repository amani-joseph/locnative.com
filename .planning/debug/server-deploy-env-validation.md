...
- verification: `pnpm --filter @locnative/server check-types` PASSES; `pnpm --filter @locnative/api check-types` PASSES. Web app tsc shows only 5 pre-existing, unrelated errors (children props, `cloudflare:workers` module, geometry undefined) — none touch serverEnv/auth/db. Consumer-facing types unchanged. END-TO-END CONFIRMED: `pnpm run deploy` for apps/server passed Cloudflare startup validation (Worker Startup Time 148ms) and published to api.locnative.com; runtime checks pass (`GET /` 200 `{"ok":true,"service":"locnative-server"}`, CORS preflight 204, `GET /api/auth/ok` 200). apps/web also deployed clean to locnative.com (`GET /` and `/coverage` 200). Fix committed (28c3581) and pushed to origin/master.
