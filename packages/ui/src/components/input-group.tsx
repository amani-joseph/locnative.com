...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
	return (
		// biome-ignore lint/a11y/useSemanticElements: a styling wrapper that groups an input with addons; role="group" conveys the grouping without imposing <fieldset> layout.
		<div
			className={cn(
...
function InputGroupAddon({
	...
	return (
		// biome-ignore lint/a11y/useSemanticElements: addon is a styling slot grouped with the input; role="group" suffices without <fieldset>.
		// biome-ignore lint/a11y/useKeyWithClickEvents: click forwards focus to the sibling input as a convenience; keyboard users tab straight to the input, so no key handler is needed.
		// biome-ignore lint/a11y/noNoninteractiveElementInteractions: the addon is decorative; the click only redirects focus to the real control.
		<div
