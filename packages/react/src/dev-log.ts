 ...
 * error state in the UI (e.g. a "Try again" label). That makes genuinely
 * unexpected failures — a misconfigured client, a network/CORS problem, or the
 * SDK throwing before a request is even sent — nearly invisible to developers.
 *
 * This logs such errors loudly during development and no-ops in production. It
 * deliberately skips `AbortError` (an expected outcome of debounced/cancelled
 * requests) so the console isn't flooded by normal cancellations.
 */
